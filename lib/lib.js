...
function moveDefence(evt){
	...
	var xx=evt.clientX-500;
	var yy=evt.clientY-400;
	if( yy>=0 && xx>=0)//第一右下象限
	{
...
function check()
{
	...
	{ 
		...
		if(distance<=63 && distance>=57 && 
			( (angel>defence.start && angel<defence.end)|| 
			  (angel+2*Math.PI>defence.start && angel+2*Math.PI<defence.end) ||
			  (angel-2*Math.PI>defence.start && angel-2*Math.PI<defence.end)
			) 
		   ) 
		{
